...
```
删除：drop procedure 存储过程名;（要一个个删除，不能一次删除多个）
```
...
区别：函数有且仅有一个返回，存储过程可以有0、1、多个返回。存储过程适合做批量插入、更新。函数适合处理数据返回一个结果。
...
```
iterate 类似于continue,继续结束本次循环，继续下一次；
